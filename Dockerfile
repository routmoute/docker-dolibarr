ARG ARCH=
ARG PHP_VERSION=8.4
ARG DOLIBARR_VERSION=23.0.0

FROM ${ARCH}php:${PHP_VERSION}-fpm-alpine

# Install dependencies and PHP extensions
RUN apk --no-cache update && apk --no-cache upgrade && \
    apk --no-cache add build-base autoconf zlib-dev libpng-dev libjpeg-turbo-dev freetype-dev krb5-dev icu-dev gettext-dev libzip-dev imap-dev libpq-dev openldap-dev \
    nginx libpng libjpeg-turbo freetype gettext libzip imap c-client libpq openldap icu-libs && \
    ...
    apk --no-cache del build-base autoconf zlib-dev libpng-dev libjpeg-turbo-dev freetype-dev krb5-dev icu-dev gettext-dev libzip-dev imap-dev libpq-dev openldap-dev && \
    rm -rf /var/cache/apk/*
...
RUN wget https://github.com/Dolibarr/dolibarr/archive/refs/tags/${DOLIBARR_VERSION}.tar.gz -O - | tar -xz && \
    ...
    chmod -R 700 /var/www/dolibarr/documents /var/www/dolibarr/htdocs/custom
